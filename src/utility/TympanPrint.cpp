...
#include <Arduino.h>
#include "TympanPrint.h"



size_t TympanPrint::write(const uint8_t *buffer, size_t size)
{
	size_t count = 0;
...
size_t TympanPrint::print(const String &s)
{
	uint8_t buffer[33];
	size_t count = 0;
...
size_t TympanPrint::print(long n)
{
	uint8_t sign=0;
...
size_t TympanPrint::println(void)
{
	uint8_t buf[2]={'\r', '\n'};
	return write(buf, 2);
}
...
{
	((class TympanPrint *)file)->write((uint8_t *)ptr, len);
	return len;
}
}

int TympanPrint::printf(const char *format, ...)
{
	va_list ap;
	va_start(ap, format);
...
int TympanPrint::printf(const __FlashStringHelper *format, ...)
{
	va_list ap;
	va_start(ap, format);
#ifdef __STRICT_ANSI__
...
size_t TympanPrint::printNumberDec(unsigned long n, uint8_t sign)
{
        uint8_t buf[11], *p;
        uint32_t digit;
...
size_t TympanPrint::printNumberHex(unsigned long n)
{
...
size_t TympanPrint::printNumberBin(unsigned long n)
{
        uint8_t buf[32], *p;
...
size_t TympanPrint::printNumberAny(unsigned long n, uint8_t base)
{
...
size_t TympanPrint::printNumber(unsigned long n, uint8_t base, uint8_t sign)
{
	uint8_t buf[34];
	uint8_t digit, i;
...
size_t TympanPrint::printFloat(double number, uint8_t digits) 
{
	uint8_t sign=0;
	size_t count=0;
