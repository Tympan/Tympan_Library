...
int makeAudioConnections(void) { //call this in setup() or somewhere like that
  ...
  //send the audio out
  patchCord[count++] = new AudioConnection_F32(compBroadband, 0, i2s_out, EarpieceShield::OUTPUT_LEFT_TYMPAN);    //First AIC, left output
  patchCord[count++] = new AudioConnection_F32(compBroadband, 0, i2s_out, EarpieceShield::OUTPUT_RIGHT_TYMPAN);   //First AIC, right output
  patchCord[count++] = new AudioConnection_F32(compBroadband, 0, i2s_out, EarpieceShield::OUTPUT_LEFT_EARPIECE);  //Second AIC (Earpieces!), left output
  patchCord[count++] = new AudioConnection_F32(compBroadband, 0, i2s_out, EarpieceShield::OUTPUT_RIGHT_EARPIECE); //Secibd AIC (Earpieces!), right output
  ...
  patchCord[count++] = new AudioConnection_F32(earpieceMixer, earpieceMixer.LEFT, audioSDWriter, 0);       //connect raw input audio to SD writer (left channel)
