/*
  ...
  Purpose: Implements multi-band WDRC compressor using the earpieces
...
void setupTympanHardware(void) {
  ...
  //Choose the desired audio input
  if (0) {
    //default to the digital PDM mics inside the earpieces
  ...
  earpieceMixer.setInputGain_dB(myState.earpieceMixer->inputGain_dB);
...
void setup() {
  ...
  Serial.println("WDRC_XBand_Earp_wApp: setup():...");
...
//float setInputGain_dB(float gain_dB) { return myState.earpieceMixer->inputGain_dB = myTympan.setInputGain_dB(gain_dB); }
...
void printGainSettings(void) {
  ...
  Serial.print("Input PGA = "); Serial.print((myState.earpieceMixer)->inputGain_dB,1);
