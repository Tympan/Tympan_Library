...
void SerialManager::createTympanRemoteLayout(void) {
  ...
  page_h = myGUI.addPage("4-Channel Recorder");
      ...
      card_h = page_h->addCard("Select Input");
          ...
          card_h->addButton("Jacks as Mic",   "W", "configMIC",   12);  //displayed string (blank for now), command (blank), button ID, button width (out of 12)
          card_h->addButton("Jacks as Line-In","e", "configLINE",  12);  //displayed string, command, button ID, button width (out of 12) 
