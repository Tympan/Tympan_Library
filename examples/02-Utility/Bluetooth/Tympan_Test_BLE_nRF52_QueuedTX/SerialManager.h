...
String str_queueNotSupported = "queueMessage() not supported by your BLE module firmware.  Using sendMessage().";
...
void setButtonState(const String &btnId, bool newState) {
  ...
  if (flag_useBleQueue) {
    ...
    Serial.println("SerialManager: setButtonState: " + str_queueNotSupported);
    ble.sendMessage(msg);
  }
}
void setButtonText(const String &btnId, String text) {
  ...
  if (flag_useBleQueue) {
    ...
    Serial.println("SerialManager: setButtonText:  " + str_queueNotSupported);
    ble.sendMessage(msg);
  }
}

void sendLotsOfData(int approx_num_bytes) {
  int total_sent = 0;
  uint8_t packet_counter = 0;
  String body = "-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  while (total_sent < approx_num_bytes) {
    //compose the message string
    String msg = "_";  //lead off with this character
    if (packet_counter < 100) msg += "0"; //add leading zero, if needed
    if (packet_counter < 10)msg += "0";   //add another leading zero, if needed
    msg += String(packet_counter);        //add the packet counter just for fun
    msg += body;                          //add the bulk of the message
...
void SerialManager::printHelp(void) {  
  ...
  Serial.println("  : Special commands related to the BLE module's TX queue:");
  Serial.println("    d/D: BLE QUEUE: Set delay to (d) 10 msec or (D) 99 msec");
  Serial.println("    q  : BLE QUEUE: Get the current TX delay between messages");
  ...
  switch (c) {
   ...
   case 'k':
    ...
    case 'v':
      ...
      break; 
    case 'd':
      val = ble.setTxQueueDelay_msec(10);
      Serial.println("serialManager: BLE Module's Queue: set delay to 10 msec (Reply = " +String(val) + ")");
      break;
    case 'D':
      val = ble.setTxQueueDelay_msec(99);
      Serial.println("serialManager: BLE Module's Queue: set delay to 99 msec (Reply = " +String(val) + ")");
      break;
    case 'q':
      val = ble.getTxQueueDelay_msec();
      Serial.println("serialManager: BLE Module's Queue: module says delay is " +String(val) + " msec (zero or negative are errors)");
      break;
    case 'w':
      val = ble.getTxQueueNumMsgs();
      Serial.println("serialManager: BLE Module's Queue: " +String(val) + " messages in queue");
      break;
    case 'W':
      val = ble.getTxQueueFreeBytes();
      Serial.println("serialManager: BLE Module's Queue: " +String(val) + " free bytes in queue");
      break;
    case 'e':
      val = ble.clearTxQueue();
...
void SerialManager::printTympanRemoteLayout(void) {
    ...
    if (flag_useBleQueue) {
      ...
      Serial.println("SerialManager::printTympanRemoteLayout: "  + str_queueNotSupported);
      ble.sendMessage(s);
    }
    setFullGUIState();
}
