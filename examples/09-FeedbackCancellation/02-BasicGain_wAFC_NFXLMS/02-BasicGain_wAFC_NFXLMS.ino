...
// some functions
settings_AFC_NFXLMS afc_settings = AudioFeedbackCancelNFXLMS_F32::getDefaultSettings(sample_rate_Hz, audio_block_samples);
void setupAFC(void) {
  afc_settings.rho = 0.0072119585;  //forgetting factor
  afc_settings.eps = 0.000919300;   //minimum power threshold (for avoiding divide-by-zero during quiet passages)
  afc_settings.mu = 0.004607254;    //step size
  afc_settings.alf = 0.000010658;   //band-limit update
  afc_settings.afl = (int)42.0f * (sample_rate_Hz / 24000.f);  //adaptive filter length (42 for 24kHz sample rate)
  
  afc.setParams(afc_settings);
  afc.setEnable(true);  //AFC will beenabled on startup (if set to true).  Set to false to disable AFC
}
