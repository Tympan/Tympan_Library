// Tympan_Earpieces
//
// Created: Chip Audette, Open Audio, March 2020
//
...
AudioConnection_F32       patchCord1(inputMixer_L, 0, hp_filt1, 0);   //connect the Left input to the left highpass filter
AudioConnection_F32       patchCord2(inputMixer_R, 0, hp_filt2, 0);   //connect the Right input to the right highpass filter
...
AudioConnection_F32           patchcord11(comp1, 0, i2s_out, 0); //First AIC, left output
AudioConnection_F32           patchcord12(comp2, 0, i2s_out, 1); //First AIC, right output
AudioConnection_F32           patchcord13(comp1, 0, i2s_out, 2); //Second AIC (Earpiece!), left output
AudioConnection_F32           patchcord14(comp2, 0, i2s_out, 3); //Secibd AIC (Earpiece!), right output
...
String overall_name = String("Tympan: TrebBoostComp with PDM Mics");
...
float incrementHPCutoffFreq_Hz(float increment_frac) {
  return setHPCutoffFreq_Hz(increment_frac*hp_cutoff_Hz);
}
float setHPCutoffFreq_Hz(float cutoff_Hz) {
  ...
  myState.hp_cutoff_Hz = hp_cutoff_Hz;
